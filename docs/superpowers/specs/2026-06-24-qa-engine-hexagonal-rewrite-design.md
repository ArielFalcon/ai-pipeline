...
This document specifies a full rewrite of `qayaba` into a new `qa-engine/`
...
`qayaba` today is a working but structurally entangled QA engine whose entire
...
```
qayaba/                               # project root — renamed from "qayaba" everywhere
...
- **Five cross-cutting cleanups (§2.3):** unify naming under `qayaba` (C1); tests in a mirror `test/` tree with path aliases (C2); deep comment cleanup (C3); code-mode ports ready in v1 with deep impl deferred to stage 2 (C4); SOLID/DRY/KISS/YAGNI throughout, no regressions (C5).
...
- **C1 qayaba-rename checklist.** Complete set of rename sites to sweep in a single
  ...
  - `QAYABA_REPO` and `QAYABA_ROOT` env vars (all references in `src/`, compose,
    and docs)
